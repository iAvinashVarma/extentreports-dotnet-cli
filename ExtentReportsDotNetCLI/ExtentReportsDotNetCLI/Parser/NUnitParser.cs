    ...
    internal class NUnitParser : IParser
    {
        ...
        public void ParseTestRunnerOutput(string resultsFile)
        {
            ...
            var suites = doc
                ...
                .Where(x => new string[] { "TestFixture", "TestCollection" }.Any(t => t.Equals(x.Attribute("type").Value, StringComparison.CurrentCultureIgnoreCase)));
        ...
        private static string GetTestName(XElement ts)
        {
            ...
            return testName.Replace("Test collection for ", string.Empty).Replace("\\\"", string.Empty).Replace("\\\\\\\\", "\\");
        }

        private static ExtentTest CreateNode(XElement tc, ExtentTest test)
        {
            var name = GetTestName(tc);
            var descriptions =
        ...
        private void AddSystemInformation(XDocument doc)
        {
            ...
            AddSystemInfo(env, "NUnit Version", $"{env.Attribute("os-version")?.Value}");
            AddSystemInfo(env, "OS Version", $"{env.Attribute("os-version")?.Value}");
            AddSystemInfo(env, "Platform", $"{env.Attribute("platform")?.Value}");
            AddSystemInfo(env, "CLR Version", $"{env.Attribute("clr-version")?.Value}");
            AddSystemInfo(env, "Machine Name", $"{env.Attribute("machine-name")?.Value}");
            AddSystemInfo(env, "User", $"{env.Attribute("user")?.Value}");
            AddSystemInfo(env, "User Domain", $"{env.Attribute("user-domain")?.Value}");
        }
