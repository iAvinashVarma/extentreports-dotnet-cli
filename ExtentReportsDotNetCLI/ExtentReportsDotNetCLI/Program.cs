...
namespace AventStack.ExtentReports.CLI
{
    internal class Program
    {
        ...
        [Option(ShortName = "r")]
        private List<string> Reporters { get; set; } = new List<string>() { "v3html" };
        ...
        private int ProcessSingle(string testResultsFilePath, string output, bool merge = false)
        {
            ...
            new XUnitParser(_extent).ParseTestRunnerOutput(testResultsFilePath);
            _extent.Flush();
            _filesProcessed++;
        ...
        private void InitializeReporter(ExtentReports extent, string path)
        {
            if (Reporters.Contains("html"))
            {
                ...
                var htmlReporter = new ExtentHtmlReporter(output);
                htmlReporter.Config.Theme = Theme.Dark;
                htmlReporter.Config.DocumentTitle = !string.IsNullOrWhiteSpace(Title) ? $"{Title} Report" : htmlReporter.Config.DocumentTitle;
                htmlReporter.Config.ReportName = !string.IsNullOrWhiteSpace(Title) ? $"{Title} Report" : htmlReporter.Config.ReportName;
            ...
            if (Reporters.Contains("v3html"))
            {
                ...
                var htmlReporter = new ExtentV3HtmlReporter(output);
                htmlReporter.Config.Theme = Theme.Dark;
                htmlReporter.Config.DocumentTitle = !string.IsNullOrWhiteSpace(Title) ? $"{Title} Report" : htmlReporter.Config.DocumentTitle;
                htmlReporter.Config.ReportName = !string.IsNullOrWhiteSpace(Title) ? $"{Title} Report" : htmlReporter.Config.ReportName;
